<template>
  <div class="app-container">
    <div class="filter-container">
      ...
      <el-select v-model="queryParams.display" class="filter-item search-item select" clearable filterable
                 placeholder="是否显示">
        <el-option label="显示" value="1"></el-option>
        <el-option label="不显示" value="0"></el-option>
      </el-select>
      <coin-Select ref="coin" @coin="checkedCoin"></coin-Select>
      ...
      <el-button v-hasPermission="['voteConfig:add']" class="filter-item" type="warning" plain @click="add">
        添加
      </el-button>
    </div>
    ...
    >
      ...
      <el-table-column label="创世序号" :show-overflow-tooltip="true" align="center" min-width="100px">
        <template slot-scope="scope">
          <span>{{ scope.row.defiId }}</span>
        </template>
      </el-table-column>

      <el-table-column label="发起地址" :show-overflow-tooltip="true" align="center" min-width="100px">
        <template slot-scope="scope">
      ...
      <el-table-column label="是否显示" :show-overflow-tooltip="true" align="center" min-width="100px">
        <template slot-scope="scope">
          <span>{{ scope.row.display | displayOf}}</span>
        </template>
      </el-table-column>

      <el-table-column label="单票扣除数量" :show-overflow-tooltip="true" align="center" min-width="120px">
        <template slot-scope="scope">
          <span>{{ scope.row.price  }}</span>
        </template>
      </el-table-column>
      <el-table-column label="投票数量" :show-overflow-tooltip="true" align="center" min-width="120px">
        <template slot-scope="scope">
          <span>{{ scope.row.voteCount  }}</span>
        </template>
      </el-table-column>
      <el-table-column label="基数" :show-overflow-tooltip="true" align="center" min-width="120px">
        <template slot-scope="scope">
          <span>{{ scope.row.baseCount  }}</span>
        </template>
      </el-table-column>
      <el-table-column label="达标数量" :show-overflow-tooltip="true" align="center" min-width="120px">
        <template slot-scope="scope">
          <span>{{ scope.row.standardCount }}</span>
        </template>
      </el-table-column>
      ...
      <el-table-column label="操作" align="center" min-width="150px" class-name="small-padding fixed-width">
        <template slot-scope="{row}">
          <el-tooltip v-hasPermission="['voteConfig:update']" class="item" effect="dark" content="修改" placement="top">
            <i class="el-icon-setting table-operation" style="color: #2db7f5;"
               @click="edit(row)"/>
          </el-tooltip>
          <el-tooltip v-hasPermission="['voteConfig:view']" class="item" effect="dark" content="详情" placement="top">
...
<script>
    ...
    import { getVoteConfig } from '@/api/index';
    ...
    export default {
        ...
        methods: {
            ...
            fetch(params = {}) {
                ...
                this.loading = true;
                getVoteConfig(params).then(response => {
                    this.loading = false;
