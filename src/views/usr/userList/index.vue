<template>
  <div class="app-container">
    <div class="filter-container">
      ...
      <el-input v-model="queryParams.phone" placeholder="手机号码" class="filter-item search-item"/>
      <el-input v-model="queryParams.email" placeholder="邮箱" class="filter-item search-item"/>
      <el-input v-model="queryParams.email" placeholder="推荐人" class="filter-item search-item"/>
...
<!--      </el-dropdown>-->
    ...
    >
      ...
      <el-table-column label="手机号码" :show-overflow-tooltip="true" align="center" min-width="120px">
        <template slot-scope="scope">
          <span>{{ scope.row.phone }}</span>
        </template>
      </el-table-column>
      <el-table-column label="邮箱" :show-overflow-tooltip="true" align="center" min-width="120px">
        <template slot-scope="scope">
          <span>{{ scope.row.email }}</span>
        </template>
      </el-table-column>
      <el-table-column label="是否锁定" :show-overflow-tooltip="true" align="center" min-width="80px">
        <template slot-scope="scope">
          <el-switch v-model="scope.row.isRecommend" @change="locking(scope.row)">
          </el-switch>
        </template>
      </el-table-column>
      <el-table-column label="vip等级" :show-overflow-tooltip="true" align="center" min-width="120px">
        <template slot-scope="scope">
          <span>{{ scope.row.language | langFilter }}</span>
        </template>
      </el-table-column>
      <el-table-column label="注册时间" :show-overflow-tooltip="true" align="center" min-width="120px">
        <template slot-scope="scope">
          <span>{{ scope.row.createTime | toTime}}</span>
        </template>
      </el-table-column>
      <el-table-column label="实名认证" :show-overflow-tooltip="true" align="center" min-width="70px">
        <template slot-scope="scope">
          <span>{{ scope.row.isRecommend | statusFilter}}</span>
        </template>
      </el-table-column>
<!--      <el-table-column label="绑定微信" prop="createTime" align="center" min-width="70px">-->
<!--        <template slot-scope="scope">-->
<!--          <span>{{ scope.row.isRecommend | statusFilter}}</span>-->
<!--        </template>-->
<!--      </el-table-column>-->
      <el-table-column label="绑定Google" prop="createTime" align="center" min-width="80px">
        <template slot-scope="scope">
          <span>{{ scope.row.isRecommend | statusFilter}}</span>
        </template>
      </el-table-column>
      <el-table-column label="推荐人" prop="createTime" align="center" min-width="130px">
        <template slot-scope="scope">
          <span>{{ scope.row.name }}</span>
        </template>
      </el-table-column>
      <el-table-column label="推荐码" prop="createTime" align="center" min-width="70px">
        <template slot-scope="scope">
...
<!--               @click="view(row)"/>-->
<!--          </el-tooltip>-->
          <el-tooltip class="item" effect="dark" content="登录日志" placement="top">
            <i v-hasPermission="['user:view']" class="el-icon-map-location table-operation"
               @click="view(row)"/>
          </el-tooltip>
          <el-tooltip class="item" effect="dark" content="解除Google验证器" placement="top">
            <i v-hasPermission="['user:view']" class="el-icon-unlock table-operation" style="color: #f4c13c;"
               @click="relieveGoogle(row)"/>
          </el-tooltip>
          <el-tooltip class="item" effect="dark" content="登录用户" placement="top">
            <i v-hasPermission="['user:view']" class="el-icon-user-solid table-operation" style="color: #f4c13c;"
               @click="login(row)"/>
          </el-tooltip>
